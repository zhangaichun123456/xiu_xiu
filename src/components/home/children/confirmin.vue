<template>
	<div class="confirmin">
		<Breadcrumb separator=">">
	        ...
	        <Breadcrumb-item>导入确认</Breadcrumb-item>
	    </Breadcrumb>
	    <div>
	        <p v-if="errflag==0" class="tip">你一共上传了{{updata.length}}条数据</p>
            <p v-if="errflag==1" class="tip">当前表格中有{{errornum}}条错误，请修改完成后重新上传</p>
	        <Table :columns="columnsname" :data="updata"></Table>
        </div>
        <footer>
	        <Button type="primary" @click="goinout" class="savebtn">重新上传</Button>
	        <Button v-if="errflag==0" type="primary" @click="gohome" class="savebtn">保存</Button>
	        <Button v-if="errflag==1" disabled type="primary" class="btn">保存</Button>
        </footer>
	</div>
</template>
<script>
