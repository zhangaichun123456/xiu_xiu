			...
			<div class="left">
				...
				<div class="candidate">
					...
					<ul v-for="item in alldpt" :key="item.name">
					   <editmemberdpttree :model="item"></editmemberdpttree>
					</ul>
				</div>
			</div>
			<div class="right">
				...
				<div class="choosekdpt">
					<ul>
						<li v-for="dpt in this.editchoosedpt1">{{dpt.name}}</li>
					</ul>
				</div>
			</div>
		</div>
...
import editmemberdpttree from './editmemberdpttree.vue'
			...
			if(e==true){
				...
				function resetdata(arr){
					for(var i=0;i<arr.length;i++){
					   ...
					   _this.editchoosedpt1.push(arr[i]);
						if(arr[i].children){
