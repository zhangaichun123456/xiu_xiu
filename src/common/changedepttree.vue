...
export default {
  ...
  methods: {
    ...
    getdptname (e){
      if(e==true){
        if(this.changedeptlist.indexOf(this.model)<0){
          this.changedeptlist.push(this.model);
        }else{
          return;
        }        
      }else{
        for(var i=0;i<this.changedeptlist.length;i++){
          if(this.changedeptlist[i].name==this.model.name){
            this.changedeptlist.splice(i,1);
            break;
          }
        }
      }
